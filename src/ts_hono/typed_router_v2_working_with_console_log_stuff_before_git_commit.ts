...
export class TypedRouter {
	...
	constructor() {
		...
		const methods = methodsLower.reduce((acc, cur) => {
			...
			acc[cur] = (path, paramsSchema, handler) => {
				...
				console.log('params', paramsSchema)
				...
				const encasedHandler: Handler<typeof path> = (ctx, ser) => {
					...
					// const paramsValid = Value.Check(CDT, paramsConverted)
					// const paramsValid = CDT.Check(paramsConverted)
					...
					// console.log('pConverted', paramsConverted)
					// console.log('pValid?', paramsValid)
				}
	...
	plugin(app: Router) {
		...
		console.log(this)
		console.log('plugin loop record')
