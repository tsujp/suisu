import { FormatRegistry, Kind, SchemaOptions, Static, TKind, TSchema, TypeBoxError, TypeRegistry, ValueGuard } from '@sinclair/typebox'
...
export namespace TypeGuard {
   ...
   export function TU256 (schema: unknown): schema is TU256 {
      return IsObject(schema) && schema[Kind] === 'API:U256' &&
         schema['type'] === 'integer'
   }
   // - - -
   export function TKind (schema: unknown): schema is TKind {
      return IsObject(schema) && Kind in schema &&
         typeof (schema as any)[Kind] === 'string' // TS 4.1.5: any required for symbol indexer
   }
   // - - -
   export function TSchema (schema: unknown): schema is TSchema {
      return (TU256(schema) || (TKind(schema)) && TypeRegistry.Has(schema[Kind]))
   }
}

// TypeRegistry

TypeRegistry.Set<TU256>(
   'API:U256',
   (schema, value) => ValueCheck.Check(schema, value),
)
...
TypeSystemErrorFunction.Set((schema, type) => {
   switch (schema[Kind]) {
      case 'API:U256':
         return 'Expected 256-bit unsigned integer'
   }
